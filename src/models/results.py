...
def write_overall_results(results_list, users_and_facs_df, travel_dict, output_filename='overall_results.xlsx',
    ...
    utilization_urban = [
        ...
        for results in results_list]
    # helper to scale by 100 only when value is present
    def scale_pct(val):
        return None if val is None else val * 100

    utilization_1 = []
    for i in range(nr_of_results):
        p_all = scale_pct(safe_percentile(utilization_all[i], percentiles[0]))
        p_rur = scale_pct(safe_percentile(utilization_rural[i], percentiles[0]))
        p_urb = scale_pct(safe_percentile(utilization_urban[i], percentiles[0]))
        utilization_1 += [p_all, p_rur, p_urb, None]
    utilization_2 = []
    for i in range(nr_of_results):
        p_all = scale_pct(safe_percentile(utilization_all[i], percentiles[1]))
        p_rur = scale_pct(safe_percentile(utilization_rural[i], percentiles[1]))
        p_urb = scale_pct(safe_percentile(utilization_urban[i], percentiles[1]))
        utilization_2 += [p_all, p_rur, p_urb, None]
    utilization_3 = []
    for i in range(nr_of_results):
        p_all = scale_pct(safe_percentile(utilization_all[i], percentiles[2]))
        p_rur = scale_pct(safe_percentile(utilization_rural[i], percentiles[2]))
        p_urb = scale_pct(safe_percentile(utilization_urban[i], percentiles[2]))
        utilization_3 += [p_all, p_rur, p_urb, None]

    # write the excel file
    data = {'Budget': budget_string_list, 'Region': regions, 'Overall access [%]': access,
            'Travel distance [miles] - p' + str(percentiles[0]): distances_1,
            'Travel distance [miles] - p' + str(percentiles[1]): distances_2,
            'Travel distance [miles] - p' + str(percentiles[2]): distances_3,
...
def save_overall_results(users_and_facs_df, travel_dict, users, facs, output_filename='overall_results.xlsx',
                         ...
                         cap_factor=1.0, cutoff=0.2, max_access=False, main_threads=1, main_tolerance=5e-3,
...
def write_fairness_results(results_list, users_and_facs_df, travel_dict, output_filename='fairness_results.xlsx',
    ...
    for results in results_list:
        ...
        wm_list += [scale(wm_all), scale(wm_rural), scale(wm_urban), None]
        wstd_list += [scale(wstd_all), scale(wstd_rural), scale(wstd_urban), None]
        wcv_list += [scale(wcv_all), scale(wcv_rural), scale(wcv_urban), None]
        dof, dof_prime = get_dof(results, users_and_facs_df, travel_dict)
        dof_list += [scale(dof), None, None, None]
        dof_prime_list += [scale(dof_prime), None, None, None]
...
def save_fairness_results(users_and_facs_df, travel_dict, users, facs, output_filename='fairness_results.xlsx',
                          output_abs_path=None,  budget_factor_list=None, strict_assign_to_one=False, cap_factor=1.0,
...
def save_greedy_results(users_and_facs_df, travel_dict, instance_dict=None, output_filename='greedy_results.xlsx',
                        output_abs_path=None, budget_factor=1.0, strict_assign_to_one=False, cap_factor=1.0, cutoff=0.2,
    ...
    for instance_size in instance_dict:
        for instance_nr in instance_dict[instance_size]:
            ...
            if max(instance_size) <= 1.0:
                ...
                if results:  # Only append if the result is not empty (i.e., a feasible solution was found)
...
def save_strict_vs_loose_results(users_and_facs_df, travel_dict, users, facs,
                                 output_filename='strict_vs_loose_results.xlsx', output_abs_path=None,
                                 budget_factor=1.0, cap_factor=1.0, cutoff=0.2, max_access=False, main_threads=1,
