...
def solve_greedily(users_and_facs_df, travel_dict, users, facs, lb=0.0, budget_factor=1.0, cap_factor=1.5,
    ...
    cap_dict = {j: cap_factor * pd.to_numeric(users_and_facs_df.at[j, 'capacity'], errors='coerce') for j in facs}
    best_open_facs = sorted_users_and_facs_df.index[:budget]
    best_obj = sum(pd.to_numeric(users_and_facs_df['capacity'], errors='coerce'))
    ...
    # Build travel matrix defensively: some user-facility pairs may be missing
    travel_matrix = np.zeros((nr_of_users, nr_of_facs))
    for i in range(nr_of_users):
        u = users[i]
        user_travel = travel_dict.get(u, {})
        for j in range(nr_of_facs):
            f = facs[j]
            travel_matrix[i][j] = user_travel.get(f, 0)
    ...
    while time()-start_time <= time_limit:
        ...
        if it_ctr == iteration_limit or best_gap < tolerance:
            break
        # update open facilities (only if assignment was feasible and returned utilization data)
        if not assignment_results or 'utilization' not in assignment_results:
            break
