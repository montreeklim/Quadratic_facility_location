...
def assign_to_one_cstr(m, i):
    # Check if there are any travel pairs for this user BEFORE creating the sum
    travel_pairs_for_user = [(i2, j) for (i2, j) in m.travel_pairs if i2 == i]
    if not travel_pairs_for_user:
        return pyo.Constraint.Feasible
    
    travel_terms = sum(m.x[(i, j)] for (i2, j) in travel_pairs_for_user)
    if m.strict_assign_to_one:
        return travel_terms == 1
    else:
        return travel_terms <= 1
...
def build_model(users_and_facs_df, travel_dict, users, facs, budget_factor=1.0, strict_assign_to_one=False,
                cap_factor=1.0, cutoff=0.0, max_access=False):
    """
    ...
    # Only include (user, facility) pairs that exist in the travel dict and clear the cutoff
    m.travel_pairs = pyo.Set(initialize=[
        (i, j)
        for i in m.users
        for j in m.facs
        if j in m.travel_dict.get(i, {}) and m.travel_dict[i][j] > m.cutoff.value
    ])
...
def build_postprocessing_model(model, results_mainstep):
    ...
    # Only include travel pairs that exist in the travel dict (guard against sparse entries)
    m.travel_pairs = pyo.Set(initialize=[
        (i, j)
...
def optimize_postprocessing_model(m, results_mainstep, threads=1, tolerance=0.0, print_sol=False, log_file=None,
    ...
    # Only scan travel_pairs that were actually created to avoid missing indices
